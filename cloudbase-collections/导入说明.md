...
已生成9个JSON Lines格式文件，对应官网国内用户的9个数据库集合：
...
6. ✅ `web_referral_links.json` - 邀请链接表
7. ✅ `web_referral_clicks.json` - 邀请点击表
8. ✅ `web_referral_relations.json` - 邀请关系表
9. ✅ `web_referral_rewards.json` - 邀请奖励表（会员天数）
...
3. **导入集合（每个文件重复以下步骤）**
   ...
   **重复以上步骤导入剩余8个集合:**
   ...
   - `web_referral_links.json` → 创建集合 `web_referral_links`
   - `web_referral_clicks.json` → 创建集合 `web_referral_clicks`
   - `web_referral_relations.json` → 创建集合 `web_referral_relations`
   - `web_referral_rewards.json` → 创建集合 `web_referral_rewards`
...
**web_referral_links（邀请链接表）**
- `creator_user_id`: 邀请人用户ID
- `share_code`: 对外分享码（唯一）
- `tool_slug`: 目标工具

**web_referral_clicks（邀请点击表）**
- `share_code`: 点击对应的分享码
- `registered_user_id`: 被归因注册用户（可为空）

**web_referral_relations（邀请关系表）**
- `inviter_user_id`: 邀请人
- `invited_user_id`: 被邀请人（唯一）
- `activated_at`: 首次激活时间

**web_referral_rewards（邀请奖励表）**
- `reward_type`: 奖励类型（signup_inviter 等）
- `amount`: 奖励会员天数（不是积分）
- `reference_id`: 幂等键（唯一）
