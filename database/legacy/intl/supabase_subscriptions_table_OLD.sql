...
-- 添加注释
COMMENT ON TABLE subscriptions IS '用户订阅信息表（会员管理）';
COMMENT ON COLUMN subscriptions.user_id IS '用户ID（关联到 auth.users）';
COMMENT ON COLUMN subscriptions.user_email IS '用户邮箱（唯一标识）';
COMMENT ON COLUMN subscriptions.platform IS '订阅平台（web/ios/android等）';
COMMENT ON COLUMN subscriptions.payment_method IS '支付方式（stripe/paypal/alipay/wechat）';
COMMENT ON COLUMN subscriptions.plan_type IS '套餐类型（pro/team）';
COMMENT ON COLUMN subscriptions.billing_cycle IS '计费周期（monthly/yearly）';
COMMENT ON COLUMN subscriptions.status IS '订阅状态（active/expired/cancelled/pending）';
COMMENT ON COLUMN subscriptions.purchase_time IS '购买时间（订单创建时间）';
COMMENT ON COLUMN subscriptions.start_time IS '付费开始时间（订阅生效时间）';
COMMENT ON COLUMN subscriptions.expire_time IS '结束时间（订阅到期时间）';
COMMENT ON COLUMN subscriptions.auto_renew IS '是否自动续费';
COMMENT ON COLUMN subscriptions.auto_renew_method IS '自动续费支付方式';
COMMENT ON COLUMN subscriptions.next_billing_date IS '下次扣费日期';
COMMENT ON COLUMN subscriptions.benefits IS '会员权益（JSON 格式）';
COMMENT ON COLUMN subscriptions.stripe_session_id IS 'Stripe Session ID';
COMMENT ON COLUMN subscriptions.stripe_subscription_id IS 'Stripe Subscription ID（自动续费用）';
COMMENT ON COLUMN subscriptions.paypal_order_id IS 'PayPal Order ID';
COMMENT ON COLUMN subscriptions.paypal_subscription_id IS 'PayPal Subscription ID（自动续费用）';
COMMENT ON COLUMN subscriptions.alipay_order_id IS 'Alipay Order ID';
COMMENT ON COLUMN subscriptions.wechat_order_id IS 'WeChat Order ID';
COMMENT ON COLUMN subscriptions.created_at IS '记录创建时间';
COMMENT ON COLUMN subscriptions.updated_at IS '记录最后更新时间';
...
-- 为 subscriptions 表添加自动更新 updated_at 的触发器
...
COMMENT ON FUNCTION check_expired_subscriptions IS '自动检查并更新过期订阅状态';
...
--
-- 测试插入数据：
-- INSERT INTO subscriptions (user_email, platform, payment_method, plan_type, status, start_time, expire_time, paypal_order_id)
